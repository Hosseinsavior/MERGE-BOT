...
async def callback_handler(c: Client, cb: CallbackQuery):
    ...
    if cb.data == "merge":
        ...
        await cb.message.edit(
            text="Okay I'll upload to drive\nDo you want to rename? Default file name is **[@Hashooorbot]_merged.mkv**",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
        ...
        await cb.message.edit(
            text="Do you want to rename? Default file name is **[@Hashooorbot]_merged.mkv**",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("👆 Default", callback_data="rename_NO"),
        ...
        if "NO" in cb.data:
            if MERGE_MODE[cb.from_user.id]==1:
                await mergeNow(
                    c,
                    cb,
                    new_file_name=f"./downloads/{str(cb.from_user.id)}/[@Hashooorbot]_merged.mkv",
                )
            elif MERGE_MODE[cb.from_user.id]==2:
                await mergeAudio(
                    c,
